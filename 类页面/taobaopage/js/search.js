$(function () {
    ...
    function Ajax(current) {
        $.ajax({
            type: "get",
            url: publicUrl + `sys/goods/goodsList${window.location.search}&current=${current}&size=10`,
    ...
    $.ajax({//点击分页，先请求出总数居的个数
        type: "get",
        url: publicUrl + `sys/goods/goodsList${window.location.search}`,
        success: function (res) {
            console.log()
            if (res.code == 200) {
    ...
    $.ajax({
        type: "get",
        url: publicUrl + "sys/goods/goodsAttribute" + window.location.search,
        success: function (res) {
            console.log(res)
            if (res.code == 200) {
                ...
                $(".nav-item .fold-up").click(function () { //点击展开或折叠
                    ...
                    if ($("#search").height() <= window.innerHeight) { //防止底部footer向上偏移
                        $("#main").height(window.innerHeight - 100 - $("#header").height())
                    } else {
                        $("#main").attr("style", "")
                    }
                })
