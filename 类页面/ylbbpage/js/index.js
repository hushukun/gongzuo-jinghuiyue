$(function () {
        ...
        success: function (res) {
            ...
            if (res.code == 200) {
                ...
                for (var i = 0; i < dataList.length; i++) {
                    ...
                    if (i < 10) {
                        naviGation += `<li><a href="views/category.html?nav=true&cid=${dataList[i].id}&id=${dataList[i].navigation_list[0].id}&level=levels&size=999">${dataList[i].typeName}</a></li>`
                    }
                }
                // console.log(dataList[i].navigation_list[0].id)
                $(".naviGation-main").eq(0).html(naviGation)
                $(".nav").eq(0).html(starrs)
                $(".navNone").map(function (index) { //定义隐藏内容的定位值
                    if (index > 5 && index < 10) {
                        $(".navNone").eq(index).css({
                            "top": "-200px"
                        })
                    } else if (index >= 10) {
                        $(".navNone").eq(index).css({
    ...
    if ($("#app").height() < window.innerHeight) {
        $("#app").height(window.innerHeight - 40)
    }
    ...
    if (document.cookie) {
        ...
        $("#cookieBtn").click(function () {
            $.removeCookie('username', { path: '/ylbbpage/' })
            $.removeCookie('avatar', { path: '/ylbbpage/' })
